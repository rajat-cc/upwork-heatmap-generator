...
def test_init_db_creates_all_tables(isolated_db):
    ...
    expected = {
        "jobs", "job_skills", "job_classifications",
        "fetch_runs", "schema_meta", "jobs_fts",
    }
...
def test_init_db_records_all_migrations(isolated_db):
    ...
    with sqlite3.connect(isolated_db) as conn:
        applied = sorted(r[0] for r in conn.execute(
...
def test_indexes_present(isolated_db):
    ...
    expected = {
        "idx_published", "idx_category", "idx_country",
        "idx_first_seen", "idx_last_fetched", "idx_disc_search",
        "idx_skill_name", "idx_class_label", "idx_class_source",
        "idx_runs_started",
    }
